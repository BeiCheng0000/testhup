                    ...
                    if passed and assertions_results:
                        for assertion_result in assertions_results:
                            if not assertion_result.get('passed', True):
                                passed = False
                                name = assertion_result.get('name', '未命名断言')
                                atype = assertion_result.get('type', '')
                                expected = assertion_result.get('expected')
                                actual = assertion_result.get('actual')
                                err = assertion_result.get('error')
                                if err:
                                    error_message = f"断言失败: {name} - {err}"
                                elif expected is not None and actual is not None:
                                    error_message = f"断言失败: {name} - 期望={expected}, 实际={actual}"
                                elif expected is not None:
                                    error_message = f"断言失败: {name} - 期望={expected}, 实际值未获取"
                                elif actual is not None:
                                    error_message = f"断言失败: {name} - 实际值={actual}, 期望值未配置"
                                else:
                                    error_message = f"断言失败: {name} - 类型={atype}, 缺少期望/实际信息"
                                break
        ...
        def execute():
            try:
                ...
                if task.task_type == 'TEST_SUITE':
                    ...
                    # 检查测试套件执行结果
                    task_success = result.get('success', False) and result.get('failed_count', 0) == 0
                ...
                # 更新执行结果
                execution_log.status = 'COMPLETED' if task_success else 'FAILED'
                execution_log.end_time = timezone.now()
                execution_log.result = result
                # 填充错误信息，优先使用汇总信息，其次使用通用error字段
                if not task_success:
                    if result.get('error_summary'):
                        execution_log.error_message = result['error_summary']
                    elif result.get('error'):
                        execution_log.error_message = result['error']
                    elif result.get('failed_count', 0) > 0:
                        # 汇总所有失败请求的错误
                        failed_errors = [
                            f"[{r.get('name')}][{r.get('method')}] {r.get('url')}: {r.get('error', '未知错误')}"
                            for r in result.get('results', [])
                            if not r.get('passed', False) and r.get('error')
                        ]
                        execution_log.error_message = '\n'.join(failed_errors) if failed_errors else f"有 {result.get('failed_count', 0)} 个请求失败，但未捕获到具体错误信息"
                    else:
                        execution_log.error_message = '任务执行失败'
                execution_log.save()
                
                # 更新任务统计
                task.update_run_stats(success=task_success)
                task.last_result = result
                task.save()
                
                logger.info("=== 开始检查发送成功通知 ===")
                # 发送通知前刷新数据库连接（线程中可能已失效）
                from django.db import close_old_connections
                close_old_connections()
                ...
                if notification_setting and notification_setting.is_enabled:
                    ...
                    # 只在任务真正成功时发送成功通知
                    if task_success and notification_setting.notify_on_success:
                        logger.info("任务执行成功，调用 _send_notification 方法发送成功通知")
                        self._send_notification(task, execution_log, success=True)
                    elif not task_success and notification_setting.notify_on_failure:
                        logger.info("任务执行失败，调用 _send_notification 方法发送失败通知")
                        self._send_notification(task, execution_log, success=False)
                    else:
                        logger.info("通知设置中未启用相应类型的通知")
                else:
                ...
                if hasattr(task, 'notification_settings'):
                    try:
                        ...
                        logger.error(f"获取任务通知设置时出错（失败情况）: {notify_e}")
                        import traceback
                        traceback.print_exc()
                else:
    ...
    def _send_notification(self, task, execution_log, success=True):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"查询自定义收件人失败，尝试重连: {db_err}")
                from django.db import close_old_connections
                close_old_connections()
                try:
                    has_custom_recipients = notification_setting.custom_recipients.exists()
                except Exception:
                    has_custom_recipients = False
                    logger.error(f"重连后仍无法查询自定义收件人，假设没有收件人")
            
            if not (has_config or has_custom_bots or has_custom_recipients):
                # 没有显式配置但不直接返回 — 邮件通知可能回退到任务创建者邮箱
                logger.warning("没有找到通知配置且无自定义设置，尝试使用任务创建者邮箱作为收件人")
                # 不直接 return，让 _send_email_notification 内部回退处理
    ...
    def _send_email_notification(self, task, execution_log, notification_setting, notification_config, success):
        ...
        try:
            ...
            if execution_log.result:
                ...
                # 提取失败请求的详细信息
                if not success and result_data.get('results'):
                    failed_items = [
                    ...
                    if failed_items:
                        ...
                        for idx, item in enumerate(failed_items, 1):
                            failed_details += f"\n  {idx}. [{item.get('method', '?')}] {item.get('name', '?')}\n"
                            failed_details += f"     URL: {item.get('url', '?')}\n"
                            if item.get('status_code') is not None:
                                failed_details += f"     状态码: {item.get('status_code')}\n"
                            if item.get('response_time') is not None:
                                failed_details += f"     响应时间: {item.get('response_time'):.0f}ms\n"
                            if item.get('error'):
                                failed_details += f"     错误: {item.get('error')}\n"
                            # 显示断言失败详情
                            assertion_results = item.get('assertions_results', [])
                            failed_assertions = [a for a in assertion_results if not a.get('passed', True)]
                            if failed_assertions:
                                failed_details += "     断言失败:\n"
                                for fa in failed_assertions:
                                    failed_details += f"       - {fa.get('name', '未命名')}: 期望={fa.get('expected')}, 实际={fa.get('actual')}"
                                    if fa.get('error'):
                                        failed_details += f", 错误={fa.get('error')}"
                                    failed_details += "\n"
            ...
            try:
                ...
                try:
                    if notification_setting.custom_recipients.exists():
                        recipients = [user.email for user in notification_setting.custom_recipients.all() if user.email]
                        logger.info(f"重试成功，使用自定义收件人: {recipients}")
                except Exception as retry_e:
                    logger.error(f"重试获取自定义收件人仍然失败: {retry_e}")
            ...
            if not recipients:
                # 回退：使用任务创建者的邮箱
                logger.warning("没有找到任何邮件收件人，尝试使用任务创建者邮箱作为回退")
                try:
                    if hasattr(task, 'created_by') and task.created_by and task.created_by.email:
                        recipients = [task.created_by.email]
                        logger.info(f"使用任务创建者邮箱作为回退: {recipients}")
                    else:
                        logger.warning("任务创建者也没有有效的邮箱地址，无法发送邮件通知")
                except Exception as e:
                    logger.error(f"获取任务创建者邮箱失败: {e}")
            
            if not recipients:
                logger.warning("最终没有找到任何邮件收件人 - 请检查通知配置")
                return
            ...
            # 根据发件邮箱域名自动选择对应的SMTP服务器
            domain = from_email.split('@')[-1].lower() if '@' in from_email else ''
            smtp_map = {
                'qq.com':          {'host': 'smtp.qq.com',           'port': 587, 'use_tls': True,  'use_ssl': False},
                'vip.qq.com':      {'host': 'smtp.qq.com',           'port': 587, 'use_tls': True,  'use_ssl': False},
                'foxmail.com':     {'host': 'smtp.qq.com',           'port': 587, 'use_tls': True,  'use_ssl': False},
                'gmail.com':       {'host': 'smtp.gmail.com',        'port': 587, 'use_tls': True,  'use_ssl': False},
                '163.com':         {'host': 'smtp.163.com',          'port': 587, 'use_tls': True,  'use_ssl': False},
                '126.com':         {'host': 'smtp.126.com',          'port': 587, 'use_tls': True,  'use_ssl': False},
                ...
                'sina.com':        {'host': 'smtp.sina.com.cn',      'port': 587, 'use_tls': True,  'use_ssl': False},
            }
            if domain and domain in smtp_map:
                smtp_cfg = smtp_map[domain]
                logger.info(f"检测到邮箱域名 {domain}，使用SMTP: {smtp_cfg['host']}:{smtp_cfg['port']}")
                connection = get_connection(
                    backend='apps.api_testing.custom_email_backend.CustomEmailBackend',
                    host=smtp_cfg['host'],
                    port=smtp_cfg['port'],
                ...
                logger.info(f"邮箱域名 {domain} 未在自动识别列表中，使用默认SMTP配置")
                send_mail(
                    subject=subject,
                    message=message,
                    from_email=from_email,
